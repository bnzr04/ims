...
<script>
    ...
    function transaction() {
        ...
        xhr.open('GET', "{{ route('manager.show-transaction') }}", true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    transaction_table.innerHTML += "<tr><td>" + row.id + "</td><td>" + row.formatted_date + "</td><td>" + row.office + "</td><td>" + row.patient_name + "</td><td>" + row.doctor_name + "</td><td>" + row.request_by + "</td><td>" + row.request_to + "</td><td>" + row.accepted_by_user_name + "</td><td>" + row.status + "</td><td><a href='/manager/requested-items/" + row.id + "' class='btn btn-outline-secondary'>View</a></td></tr>";
                }
    ...
    todayForm.addEventListener('submit', (event) => {
        ...
        xhr.open('GET', `{{ route('manager.filter-transaction') }}?today=1`);
        xhr.onload = function() {
            if (xhr.status === 200) {
                var data = JSON.parse(xhr.responseText);
                var transaction_table = document.querySelector('#transaction_table');
    ...
    yesterdayForm.addEventListener('submit', (event) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('GET', `{{ route('manager.filter-transaction') }}?yesterday=1`);
        xhr.onload = function() {
            if (xhr.status === 200) {
                var data = JSON.parse(xhr.responseText);
                var transaction_table = document.querySelector('#transaction_table');
    ...
    thisMonthForm.addEventListener('submit', (event) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('GET', `{{ route('manager.filter-transaction') }}?this-month=1`);
        xhr.onload = function() {
            if (xhr.status === 200) {
                var data = JSON.parse(xhr.responseText);
                var transaction_table = document.querySelector('#transaction_table');
