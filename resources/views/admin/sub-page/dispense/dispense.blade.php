...
<script>
    ...
    var mainUrl = "{{ route('admin.filter-dispense') }}?today=1";
                ...
                if (data.length > 0) {
                    ...
                    viewButtons.forEach(function(button) {
                        button.addEventListener("click", function() {
                            ...
                            var fetchUrl = window.APP_URL + "/admin/fetch-record/" + itemId + filterForFetchUrl;
                                ...
                                if (req.status === 200) {
                                    ...
                                    if (response.length > 0) {
                                        response.forEach(function(row) {
                                            var requestUrl = window.APP_URL + "/admin/view-request/" + row.request_id;
    ...
    let currentMainUrl = "{{ route('admin.filter-dispense') }}?today=1";
    let currentFilterForFetchUrl = "?today=1";

    /////////Today////////
    todayForm.addEventListener('submit', function(event) {
        event.preventDefault();
        mainUrl = "{{ route('admin.filter-dispense') }}?today=1";
        todayTitle();
        ...
        // console.log(currentTableTitle);
    });

    /////////Yesterday////////
    yesterdayForm.addEventListener('submit', function(event) {
        event.preventDefault();
        mainUrl = "{{ route('admin.filter-dispense') }}?yesterday=1";
        yesterdayTitle();
    ...
    thisMonthForm.addEventListener('submit', function(event) {
        event.preventDefault();
        mainUrl = "{{ route('admin.filter-dispense') }}?this-month=1";
        thisMonthTitle();
    ...
    filterForm.addEventListener('submit', (event) => {
        ...
        mainUrl = `{{ route('admin.filter-dispense') }}?date_from=${fromDateInput}&date_to=${toDateInput}`;
