...
<script>
            ...
            if (trans.status === 200) {
                ...
                if (data.length > 0) {
                    ...
                    data.forEach(function(row) {
                        var requestUrl = window.APP_URL + "/admin/view-request/" + row.id;
    ...
    var url = "{{ route('admin.filter-all-transaction') }}";
    var title = "Today transaction";
    index();

    thisDayBtn.addEventListener('click', function() {
        url = "{{ route('admin.filter-all-transaction') }}?this-day=1";
    ...
    thisWeekBtn.addEventListener('click', function() {
        url = "{{ route('admin.filter-all-transaction') }}?this-week=1";
    ...
    thisMonthBtn.addEventListener('click', function() {
        url = "{{ route('admin.filter-all-transaction') }}?this-month=1";
    ...
    filterForm.addEventListener('submit', function(event) {
        ...
        url = `{{ route('admin.filter-all-transaction') }}?filter=1&from=${dateFrom}&to=${dateTo}`;
    ...
    requestCodeSearchForm.addEventListener('submit', function(event) {
        ...
        url = `{{ route('admin.search-transaction') }}?req-code=${requestCodeInputSearch.value}`;
